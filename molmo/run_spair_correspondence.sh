...
MODEL_PATH="${MODEL_PATH:-/data/shared-vilab/pretrained_models/VLM_models/Molmo2-8B}"
...
MIN_INPUT_PIXELS="${MIN_INPUT_PIXELS:-0}"
...
for gpu_id in "${GPU_ARRAY[@]}"; do
  ...
  echo "Launching Molmo2 shard ${shard_id}/${NUM_SHARDS} on GPU ${gpu_id}"
  echo "  log: ${log_file}"
  (
    export CUDA_VISIBLE_DEVICES="${gpu_id}"
    python -u "${SCRIPT_DIR}/molmo_spair_correspondence.py" \
...
echo "Watch live: python ${PARENT_DIR}/watch_spair_progress.py --output-dir ${OUTPUT_DIR}"
monitor_pid=""
if [[ "${WATCH_PROGRESS}" == "1" ]]; then
  python "${PARENT_DIR}/watch_spair_progress.py" --output-dir "${OUTPUT_DIR}" &
  monitor_pid="$!"
fi
...
if [[ -n "${monitor_pid}" ]]; then
  ...
  python "${PARENT_DIR}/watch_spair_progress.py" \
    --output-dir "${OUTPUT_DIR}" --once || true
fi

if (( failed != 0 )); then
  echo "ERROR: one or more shards failed. Check ${OUTPUT_DIR}/shard*.log" >&2
  exit 1
fi
...
if (( NUM_SHARDS > 1 )); then
  ...
  for ((i = 0; i < NUM_SHARDS; i++)); do
    shard_file="$(printf '%s/shards/shard%02d/test_predictions.jsonl' \
      "${OUTPUT_DIR}" "${i}")"
...
python "${SCRIPT_DIR}/molmo_spair_correspondence.py" \
  --model-path "${MODEL_PATH}" \
  --dataset-root "${DATASET_ROOT}" \
  --output-dir "${OUTPUT_DIR}" \
  --split test \
