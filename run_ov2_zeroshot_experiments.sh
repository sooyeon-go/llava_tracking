#!/usr/bin/env bash
# Extract ov2_zeroshot webms to frames, then run experiment 1/2 + movement analysis.
set -uo pipefail

SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"

VIDEO_ROOT="${VIDEO_ROOT:-/data/project-vilab/minjung/Fastgen_mj/docs/reference/ov2_zeroshot}"
RESULTS_ROOT="${RESULTS_ROOT:-${SCRIPT_DIR}/ov2_zeroshot_tracking_results}"
...
videos=(
  "video_1_original.webm"
  "video_3_original.webm"
)

# Referring expressions used by Track <description>
descriptions=(
  "the cat coming forward"
  "the moving car"
)

# Short folder names under RESULTS_ROOT
sequence_names=(
  "video_1_original"
  "video_3_original"
)
...
for index in "${!videos[@]}"; do
  ...
  # No DAVIS masks for these clips; experiments only need FRAMES_DIR when set.
...
echo
echo "Frames stay next to the webms under: ${VIDEO_ROOT}"
...
echo "All ov2_zeroshot experiments completed."
